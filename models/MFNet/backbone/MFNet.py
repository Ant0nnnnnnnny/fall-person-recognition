...
class CBL(torch.nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.separate_conv = torch.nn.Conv2d(in_channels=input_channels, out_channels=input_channels, kernel_size=(
            3, 3), padding=padding, stride=stride, groups=input_channels)
...
class SPPF(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.CBL = CBL(input_channels=in_channels,
                       output_channels=out_channels)
        padding, stride = adjusted_padding_stride(5)
        self.maxpooling = torch.nn.MaxPool2d(
            kernel_size=5, padding=padding, stride=stride)
        self.CBL2 = CBL(input_channels=4 * in_channels,
                        output_channels=out_channels)

    def forward(self, x):
        y = self.CBL(x)
        m1 = self.maxpooling(y)
        m2 = self.maxpooling(m1)
        m3 = self.maxpooling(m2)
