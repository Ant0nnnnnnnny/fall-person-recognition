...
class MPIIDataset(JointsDataset):
    ...
    def _get_db(self):
        ...
        for a in anno:
            ...
            c = np.array(a['center'], dtype=np.float16)
            s = np.array([a['scale'], a['scale']], dtype=np.float16)
            ...
            joints_3d = np.zeros((self.num_joints, 3), dtype=np.float16)
            joints_3d_vis = np.zeros((self.num_joints,  3), dtype=np.float16)
